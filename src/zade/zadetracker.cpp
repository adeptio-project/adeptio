...
CzADETracker::CzADETracker(std::string strWalletFile)
{
...
CzADETracker::~CzADETracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzADETracker::Init()
{
...
bool CzADETracker::Archive(CMintMeta& meta)
{
...
bool CzADETracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzADETracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzADETracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzADETracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzADETracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzADETracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzADETracker::GetSerialHashes()
{
...
CAmount CzADETracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzADETracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzADETracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzADETracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzADETracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzADETracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzADETracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzADETracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzADETracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzADETracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzADETracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzADEWallet* zADEWallet)
{
    bool iszADEWalletInitialized = (NULL != zADEWallet);
    CMintMeta meta;
    ...
    if (! iszADEWalletInitialized)
        zADEWallet = new CzADEWallet(strWalletFile);
    meta.isSeedCorrect = zADEWallet->CheckSeed(dMint);
    if (! iszADEWalletInitialized)
        delete zADEWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzADETracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzADETracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzADETracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzADETracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzADETracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzADETracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzADEWallet* zADEWallet = new CzADEWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zADEWallet);
        }
        delete zADEWallet;
        LogPrint("zero", "%s: added %d dzade from DB\n", __func__, listDeterministicDB.size());
    }
