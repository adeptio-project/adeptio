...
CzADEWallet::CzADEWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zade seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZADESeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZADESeed_deprecated()) {
                    LogPrintf("%s: Updated zADE seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zade seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zade wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzADEWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZADECount(nCountLastUsed);
    else if (!walletdb.ReadZADECount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzADEWallet::Lock()
{
    seedMaster = 0;
}

void CzADEWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzADEWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZADE(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzADEWallet::LoadMintPoolFromDB()
{
...
void CzADEWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzADEWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzADEWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zadeTracker->HasPubcoinHash(pMint.first)) {
...
bool CzADEWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZADE(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zadeTracker which also adds to database
    pwalletMain->zadeTracker->Add(dMint, true);
...
void CzADEWallet::SeedToZADE(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzADEWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzADEWallet::GenerateDeterministicZADE(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzADEWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzADEWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzADEWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
