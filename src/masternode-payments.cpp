...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake)
{
	...
	CAmount developerfeePayment = blockValue * 0.10;
		...
		BOOST_FOREACH (CTxOut out, txNew.vout) {
			if(payee.scriptPubKey == developerfeescriptpubkey) {
					if(out.nValue >= requiredDeveloperPayment) {
						foundDevFee = true;
						LogPrintf("Developer-Fee Payment found! Thanks for supporting Adeptio project!");
					}
			}
		}
	}
...
void FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, bool fProofOfStake, bool fZADEStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    if (!pindexPrev) return;
    ...
    if (IsSporkActive(SPORK_13_ENABLE_SUPERBLOCKS) && budget.IsBudgetPaymentBlock(pindexPrev->nHeight + 1)) {
        ...
        masternodePayments.FillBlockPayee(txNew, nFees, fProofOfStake, fZADEStake);
    }
}
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake, bool fZADEStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    if (!pindexPrev) return;
	...
	BOOST_FOREACH (CTxOut out, txNew.vout) {
		if(out.scriptPubKey == developerfeescriptpubkey) {
			if(out.nValue >= requiredDeveloperPayment) {
				...
				LogPrint("masternode", "Developer-Fee Payment found! Thanks for supporting ADE!");
			}
		}
	}
