...
#ifndef ADE_ZADEWALLET_H
#define ADE_ZADEWALLET_H
...
public:
    CzADEWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZADE(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZADE(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //ADE_ZADEWALLET_H
