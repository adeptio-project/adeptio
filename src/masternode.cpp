...
void CMasternode::Check(bool forceCheck)
{
    ...
    if( (storADElastCheck >= 12 * 60 * 60) || (storADElastCheck >= 1 * 60 * 60 && activeState == MASTERNODE_STORADE_EXPIRED) ) {
        ...
        } else if(block_height >= 89000000) // Start storADE in v2
...
void CMasternode::CheckStorADEport()
{
    ...
    if( activeState == incorrect )
        ...
        LogPrintf("CMasternode::CheckStorADEport() - %s StorADE check success.\n", storade_addr.ToStringIP());
}
...
bool CMasternodeBroadcast::CheckAndUpdate(int& nDos)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN) {
        if (addr.GetPort() != 9075) return false;
    } else if (addr.GetPort() == 9075)
        return false;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 ADE tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = 0;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 ADE tx -> 1 confirmation
