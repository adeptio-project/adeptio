...
int nAnonymizeAdeptioAmount = 1000;
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\ADE
// Windows >= Vista: C:\Users\Username\AppData\Roaming\ADE
// Mac: ~/Library/Application Support/ADE
// Unix: ~/.adeptio
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ADE";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "adeptio.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty adeptio.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override adeptio.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "adeptiod.pid"));
