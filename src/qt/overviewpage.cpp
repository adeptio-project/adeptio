...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeADEAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeADEAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeADEAmount = strAnonymizeADEAmount.remove(strAnonymizeADEAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeADEAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeADEAmount * COIN) nMaxToAnonymize = nAnonymizeADEAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeADEAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeADEAmount));
        strAnonymizeADEAmount = strAnonymizeADEAmount.remove(strAnonymizeADEAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeADEAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
