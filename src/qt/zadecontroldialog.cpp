...
#include "ui_zadecontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zadeTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zadeTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zADE.";
            else if (!mint.isSeedCorrect)
                strReason = "The zADE seed used to mint this zADE is not the same as currently hold in the wallet";
            else
