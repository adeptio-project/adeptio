...
int64_t GetBlockValue(int nHeight)
{
    ...
    int64_t nSubsidy = 0;
    if (nHeight == 0) {
        nSubsidy = 45949589.90115522 * COIN;
    } else if (nHeight < 250 && nHeight > 0) {
        nSubsidy = 1 * COIN;
    } else if (nHeight < (Params().NetworkID() == CBaseChainParams::TESTNET ? 145000 : 151200) && nHeight >= 250) {
        nSubsidy = 2 * COIN;
    } else if (nHeight <= Params().LAST_POW_BLOCK() && nHeight >= 300) {
        nSubsidy = 3 * COIN;
    } else if (nHeight <= 400 && nHeight > Params().LAST_POW_BLOCK()) {
        nSubsidy = 4 * COIN;
    } else if (nHeight <=  && nHeight >= 500) {
        nSubsidy = 5 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZADEStake)
{
    ...
    if (nHeight <= 43200) {
        ...
        //When zADE is staked, masternode only gets 2 ADE
        ret = 3 * COIN;
        if (isZADEStake)
            ret = 2 * COIN;
    }
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        const bool hasADEInputs = !pivInputs.empty();
        ...
        if (isBlockFromFork) {
            ...
            if(hasZADEInputs){
                for (const CTxIn& zPivInput : zADEInputs) {
