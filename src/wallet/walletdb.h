...
// Copyright (c) 2016-2018 The ADE developers
...
public:
    ...
    bool WriteZADESeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZADESeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZADESeed_deprecated(uint256& seed);
    bool EraseZADESeed();
    bool EraseZADESeed_deprecated();

    bool WriteZADECount(const uint32_t& nCount);
    bool ReadZADECount(uint32_t& nCount);
