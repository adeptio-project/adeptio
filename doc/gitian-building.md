...
*Setup instructions for a gitian build of ADE using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the ADE
...
Clone the git repositories for adeptio and gitian and then checkout the adeptio version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/adeptio-crypto/adeptio.git
cd adeptio
...
Follow the instructions in [doc/release-process.md](release-process.md) in the adeptio repository
...
To build ADE (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the adeptio repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/adeptio/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/adeptio-crypto/adeptio
...
For example:
```bash
URL=https://github.com/crowning-/adeptio.git
...
./bin/gbuild --commit adeptio=${COMMIT} --url adeptio=${URL} ../adeptio/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/adeptio-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/adeptio-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/adeptio-build.assert
```
...
[adeptio/gitian.sigs](https://github.com/adeptio-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
